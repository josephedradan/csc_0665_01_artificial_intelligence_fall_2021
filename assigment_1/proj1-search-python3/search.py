...
def depthFirstSearch(problem) -> List[str]:
    ...
    set_position_traveled: Set[Tuple[int, int]] = set()

    def dfs(list_position: List[Tuple[Tuple[int, int], str, int]]):

        for i in list_position:

            position_new = i[0]  # (int, int)
            name_direction = i[1]  # "Direction"
            cost = i[2]  # some number idk

            if position_new not in set_position_traveled:
                list_position_new = problem.getSuccessors(position_new)  # ((int, int), "Direction", some_number_idk)

                nonlocal list_answer
                list_answer.append(name_direction)

                set_position_traveled.add(position_new)

                dfs(list_position_new)

    dfs(problem.getSuccessors(problem.getStartState()))
